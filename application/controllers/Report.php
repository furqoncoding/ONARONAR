...
class Report extends CI_Controller
{
	function __construct()
	{
		...
		if (!$this->ion_auth->logged_in()) {								// logged in
	...
	public function email($dateOrId)
	{
		$ReportByID = 0;
		if (strtotime($dateOrId) === false) {
			$ReportByID = $this->db->get_where('onar_v_report', array('id' => $dateOrId))->row();
			$date = $ReportByID->event_date;
		} else {
			$date = $dateOrId;
		}
		...
		$this->email->from('onairReport@an.tv');
		// $this->email->to('dmanager@an.tv');
		$this->email->to('onar@an.tv');
		// $this->email->cc($emailCC);
		$this->email->bcc($emailCC);
		$subject = 'On Air Report ' . $date;
		...
		$jumlah = !empty($ReportByID) ? 1 : count($multiUpdate);
		$idArrayData = !empty($ReportByID) ? array($ReportByID->id) : $multiUpdate;
		if (!isset($_POST['totalReport'])) {
			$totalReport = null;
		} else {
			$totalReport = $_POST['totalReport'];
		};

		$data['jumlah'] = $jumlah;
		$data['multiUpdate'] = $idArrayData;
		$data['date'] = $date;
		...
		if ($totalReport > 0) {
			...
			if ($multiUpdate == 0 && strtotime($dateOrId) != false) {
				$this->session->set_flashdata('error', 'Please Checklist to send report');
				redirect('report/report_list_by_date/' . $dateOrId);
			} else {
				//Send mail 
				if ($this->email->send()) {
	...
	public function report_by_id($id)
	{
		...
		$data = array(
			...
			'icon' => 'report_form_read',
			'create' => base_url() . 'index.php/report/submit_report',
			'row' => $this->Report_model->get_report_by_id($id),
			'id' => set_value('id'),
			'button' => 'Submit',
			'action' => base_url() . 'index.php/report/email/' . $id,
		);

		$this->template->main('report/report_by_id', $data);
		$this->db->where('event_date', date('d-m-Y'));
	}
